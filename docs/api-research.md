...
- **약관 주의 (2024-11 개정)**:
  ...
  - **API 데이터를 AI 모델(학습 등)에 사용 금지**. 단 Strava 공식 발표는 "사용자 본인에게
    피드백을 제공하는 코칭 플랫폼, 사용자가 자기 데이터를 이해하도록 돕는 도구는 여전히 허용"
    이라고 명시 — Runners Plan처럼 본인 데이터 기반 코칭은 허용 범위. 모델 학습에 쓰거나
    타인에게 노출하는 것이 금지선. LLM '추론'에 넣는 것은 여전히 회색지대이므로 설계 시 검토.
...
- **승인/비용**: 무료이나 **기업(비즈니스) 대상 프로그램**. 개인/취미 개발자 불가.
  ...
  - **재확인 (2026-07-14): 여전히 중단 상태.** 신청 폼은 "Under Construction", 공식 포럼 답변
    (2026-05): "프로그램 개편 중이라 신규 접수 일시 중단, 새 프로세스 확정 후 재개, 일정 미정".
    3~7월 사이 신규 승인 사례 확인 안 됨, 개발자 문의 폼도 무응답 보고 다수. 대기자 명단·알림
    채널 없음 — developer.garmin.com 을 직접 모니터링하라는 게 공식 안내.
    (비공식 python-garminconnect는 2026-03 Cloudflare 차단으로 2.5개월 전면 중단됐다가
    6월에 TLS 위장 방식으로 복구 — 프로덕션 부적합 판단 유지.)
...
주의: Strava의 AI 조항 — 본인 데이터 기반 코칭/피드백은 공식적으로 허용 범위이나, API 데이터의
모델 학습 사용은 금지. LLM 추론(플랜 생성)에 Strava 데이터를 넣는 방식은 회색지대이므로
구현 전 약관 원문(strava.com/legal/api) 재확인 권장.
